...
def gen_matrices_poly(num_rows_R, num_cols_S, tup_ratio, feat_ratio, mode):
    ...
    urg = polyglot.eval(language="R", string="source('benchUtils.r'); function(x,y,z,w){ genMatricesForPy(x,y,z,w); }")
    res = urg(num_rows_R, num_cols_S, tup_ratio, feat_ratio)

    S, Ks, Rs, matMatrixForeign, target = res
    nm = morpheus.NormalizedMatrix(S, Ks, Rs, Scheckable=False) 
    mat = morpheus.Faux(matMatrixForeign)
    target = morpheus.Faux(target)
...
def get_tasks(params, n_mat, d_mat, T, target, tasks, is_monolang):
    ...
    if is_monolang:
        ...
        urg = polyglot.eval(language="R", string="source('benchUtils.r'); function(x,y){ genRanMatrixForPy(x,y); }")
        log_reg_winit = urg(d_mat, 1)
        print(dir(morpheus))
    ...
    tasks = filter(lambda x: x[0] in tasks, all_tasks)
    return tasks
...
def main():
    ...
    mode = args.mode
    is_monolang = False#args.monolang
    print(is_monolang)
    print(mode)
    ...
    for TR in TRs:
        for FR in FRs:
            ...
            for (name, action) in task_pairs:
                ...
                times = benchmark_it(lambda : action(matrices["matMatrix"]), fname)
main()
